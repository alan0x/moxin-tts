...
```bash
# From the project root (mofa-studio/)
cargo build --package moxin-tts --release

# Or shorter form
cargo build -p moxin-tts --release

# Output: ./target/release/moxin-tts.exe (Windows) or ./target/release/moxin-tts (Unix)
```
...
cargo run -p moxin-tts -- --dataflow path/to/dataflow.yml
```
...
RUSTFLAGS="-C target-cpu=native" cargo build -p moxin-tts --release
```
...
# Strip symbols
cargo build -p moxin-tts --release
strip target/release/moxin-tts  # Unix
strip target/release/moxin-tts.exe  # Windows (requires GNU binutils)
...
# Build
cargo build -p moxin-tts --release --target x86_64-unknown-linux-gnu
```
...
cargo build -p moxin-tts --release --target x86_64-pc-windows-gnu
```
...
After building:
1. Run the application: `./target/release/moxin-tts`
...
https://github.com/alan0x/moxin-tts/issues
